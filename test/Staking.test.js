...
describe("StakingReward", function () {
    async function deployTokenFixture() {
        ...
        const tokenStakingFactory = await ethers.getContractFactory("StakingToken", deployer);
        const tokenStaking = await tokenStakingFactory.deploy();
        await tokenStaking.waitForDeployment();

        const tokenRewardFactory = await ethers.getContractFactory("RewardToken", deployer);
        const tokenReward = await tokenRewardFactory.deploy();
        await tokenReward.waitForDeployment();

        const stakingRewardsFactory = await ethers.getContractFactory("DiscreteStakingRewards", deployer);
        const stakingRewards = await stakingRewardsFactory.deploy(tokenStaking.target, tokenReward.target);
        await stakingRewards.waitForDeployment();

        return { stakingRewards, tokenStaking, tokenReward, deployer, user1, user2 };
    }

    describe("Deployment", function () {
        it("Deploying contract, sending tokens and check balances", async function () {
            const {stakingRewards, tokenStaking, tokenReward, deployer, user1, user2 } = await loadFixture(deployTokenFixture);
            await expect(
    ...
    describe("Functionality", function () {
        ...
        it("Calculate rewards earned", async function () {
            ...
            const rewardsUser1 = await stakingRewards.calculateRewardsEarned(user1.address);
            const rewardsUser2 = await stakingRewards.calculateRewardsEarned(user2.address);
        ...
        it("Claim rewards for user1 and user2 ", async function () {
            ...
            await expect(claimedRewardsUser1).to.equal(val_before_claim_usr1+rewardsUser1);
            await expect(claimedRewardsUser2).to.equal(val_before_claim_usr2+rewardsUser2);
        });
    })
})